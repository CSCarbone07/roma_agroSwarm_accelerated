...
std::array<float,3> InformationGainStrategy::pickNextTarget(Agent* ag){
  ...
  */
  ...
  {
    std::cout << std::endl;
    std::cout << "Starting IG for agent " << ownerAgent->getId() << std::endl;
  }
        ...
        || ownerAgent->GetCommunicationsRange() == -1)) 
        {
            ...
            {         
              for(Cell* c : cells_7x7)
              {
                if (c->getId() == elegibles.at(i).first->getId())
                {
                  ...
                  if(DEBUG_IG && ownerAgent->getId() == testingId)
                  {
                  std::cout << "Cell " << c->getId() << " elegible in range 7x7 for agent " << t->getId() << " from agent " << ownerAgent->getId() << std::endl;
                  }
                }
              }
            }
          ...
          if(isCellInRange==true)
          {
            for(unsigned j = 0; j<elegibles.size(); j++)
            {
              ...
              if(DEBUG_IG && ownerAgent->getId() == testingId)
              {
                std::cout << "Cell " << elegibles.at(j).first->getId() << " with " << elegibles.at(j).first->getUtility() 
