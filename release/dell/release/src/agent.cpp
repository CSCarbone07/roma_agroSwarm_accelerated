...
bool Agent::doStep(unsigned timeStep){
  ...
  switch(nextAction()){
    ...
    case SCAN:
      ...
      Cell* currentCell;      
      if (agentId != -1){
        this->cells.erase(targetId);
        this->cells.insert(std::make_pair<>(targetId, Engine::getInstance().getWorld()->getAgent(agentId)->cells.at(targetId)));
      }
